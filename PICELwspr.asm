...
;
; ****************************************************************************
; * PICELwspr - WSPR Beacon Transmitter with AD9850/AD9851 DDS on PIC-EL     *
; *  Version 1.0                                                             *
; *  February 11, 2010                                                       *
...
;
; IMPORTANT: Before transmitting with your PICEL/DDS-60 you must first
; load the WSPR message containing your station information. To generate this
...
; WSPR data.
;
; WSPR timing is critical. WSPR transmissions must begin within a second or so
; from the top of an even minute. Start the timing process by depressing the
; 'RESET' pushbutton at the top of any even minute. The internal timer will
; then start the timing process. The first beacon transmission will begin ten 
; minutes after reset and repeat every ten minutes. The transmission period is 
; approximately 110 seconds. The internal timer will normally allow operation 
; for a day or two before re-synchroniztion is necessary.
; 
...
;                           bringing in subset
;    2/11/10   PICELwspr1.0 Modified to tranmit WSPR beacons by Gene Marcus, W3PM 
;	 2/23/10   PICELwspr1.1 Added code to set start-up frequency to 0 Hz (Idle)  
;*****************************************************************************
;                                                                              
...
MCODE_REV_0  equ  '1'     ; Current code version is 1.0                     
MCODE_REV_1  equ  '.'     ;                                                   
MCODE_REV_2  equ  '0'     ;                                                   
MCODE_REV_3  equ  '-'     ;
...
;
		...
		w_temp					  ; Used by interrupt rountine to save W
		sec_count				  ; Internal counter (seconds)
		min_count				  ; Determines XMIT time interval
		sendcode				  ; Transmit flag
		status_temp				  ; Used by interrupt routine to save status
		...
		tchar					  ; Temporary variable for WSPR symbol 
		char					  ; Temporary variable for WSPR symbol						
...
IRQSVC                              ; Interrupt 2PPS starts here
		org			H'04'
		...
		decfsz		min_count		
		goto		main3
		movlw		D'10'			; Set XMIT time interval here
		movwf		min_count
...
;---------------------------------------------------------------------
		...
		movwf		T1CON
		movlw		D'011'			; Set up tmr1 for 500 uSec
		movwf		TMR1H
		movlw		D'221'
		movwf		TMR1L
		...
		movlw		D'01'			; Transmit flag reset
		movwf		sendcode
...
;
;       Display "IDLE" on line 2
;
        movlw   0xC6              ; Point LCD at digit 6 of LCD line 2               
        call    cmnd2LCD          ;
        movlw   'I'               ; digit 1              
        call    data2LCD          ;
        movlw   'D'               ; digit 2                  
...
read_encoder
		...
		movfw		sendcode	  ; Check for transmit flag
		addlw		D'0'
		bz			loop
...
;
		...
		call		wait_a_sec	  ; per WSPR protocol
